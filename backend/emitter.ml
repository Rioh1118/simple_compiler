...
and trans_stmt ast nest tenv env =
  ...
  | Assign (v, e) ->
      trans_exp e nest env ^ trans_var v nest env
      ^ "\tpopq (%rax)\n" (* iprintのコード *)
  | CallProc ("iprint", [ arg ]) ->
  ...
  | CallProc (s, el) -> (
      ...
      | FunEntry { formals = _; result = _; level } ->
          ...
          ^ List.fold_right
              ...
              el ""
          (* 静的リンクを渡すコード *) ^ passLink nest level
          (* 関数の呼出しコード *) ^ "\tcallq "
          ^ s ^ "\n"
          (* 積んだ引数+静的リンクを降ろす *)
  ...
  | Block (dl, sl) ->
      ...
      let code =
        List.fold_left
          ...
          "" sl (* 局所変数分のフレーム拡張の付加 *)
      in
      ex_frame ^ code (* elseなしif文のコード *)
  | If (e, s, None) ->
      let condCode, l = trans_cond e nest env in
      condCode ^ trans_stmt s nest tenv env
      ^ sprintf "L%d:\n" l (* elseありif文のコード *)
  | If (e, s1, Some s2) ->
